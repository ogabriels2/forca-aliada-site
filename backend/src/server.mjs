...
if (corsOrigins.length === 0) return cb(new Error(‘CORS blocked: no origins configured’));
...
const { rows } = await pool.query(
‘SELECT * FROM user_preferences WHERE user_id=$1’, [req.user.sub],
);
if (!rows.length) {
// retorna defaults
return res.json({
email_server: true, email_events: true, email_community: true,
public_profile: true, show_online: true, public_history: false, theme: ‘light’,
});
}
...
const {
email_server = true, email_events = true, email_community = true,
public_profile = true, show_online = true, public_history = false,
theme = ‘light’,
} = req.body;

await pool.query(` INSERT INTO user_preferences(user_id,email_server,email_events,email_community,public_profile,show_online,public_history,theme,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,NOW()) ON CONFLICT(user_id) DO UPDATE SET email_server=$2, email_events=$3, email_community=$4, public_profile=$5, show_online=$6, public_history=$7, theme=$8, updated_at=NOW()`,
[req.user.sub, email_server, email_events, email_community, public_profile, show_online, public_history, theme],
);
res.json({ ok: true });
});
